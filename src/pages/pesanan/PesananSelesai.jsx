...
import UlasanStarIcon from "../../assets/pesanan/UlasanStarIcon.svg";
    ...
    const fetchData = async () => {
      ...
      try {
        const q = query(
          ...
          where("userId", "==", userId),
          where("rentalStatus", "in", ["selesai", "sudah direview"])
        );
...
function PopupUlasan({ product, onClose, onComplete }) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 font-poppins">
      <div className="w-[62.5rem] h-[40.75rem] flex flex-col justify-center items-center relative bg-white rounded-3xl">
        <h1 className="text-[#003F62] font-bold text-4xl mb-4 mt-4">
          Bagaimana Produk Kami?
        </h1>
        <img src={UlasanStarIcon} alt="" className="mb-8" />
        <div className="flex gap-x-4 mb-12">
          ...
          <div>
            <input type="file" ref={fileInputRef} style={{ display: "none" }} />
            <div
              className="w-[18.5rem] pt-3 h-[6.6rem] flex flex-col justify-center items-center rounded-3xl border-2 border-dashed border-[#001622] cursor-pointer hover:opacity-70"
              onClick={handleDivClick}
            >
        ...
        <textarea
          ...
          rows="10"
          className="w-[52rem] border border-[#666E73] rounded-lg mb-2 p-2"
        ></textarea>
        <button
